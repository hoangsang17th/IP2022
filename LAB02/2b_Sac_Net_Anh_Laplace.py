...
# Định nghĩa hàm Tich_chap()
def Apply_Filter(img,mask):
    m, n = img.shape
    img_new = np.zeros([m, n])
    for i in range(1, m-1):
        for j in range(1, n-1):
            temp   =  img[i-1, j-1]    * mask[0, 0]                   +  img[i, j-1]      * mask[0, 1]                   +  img[i+1, j - 1]  * mask[0, 2]\
                                   +  img[i-1, j]      * mask[1, 0]                   +  img[i, j]        * mask[1, 1]                   +  img[i+1, j]      * mask[1, 2]\
                   +  img[i - 1, j+1]  * mask[2, 0]                   +  img[i, j + 1]    * mask[2, 1]                   +  img[i + 1, j + 1]* mask[2, 2]
            img_new[i, j]= temp
...
img_loc_Laplacian_chuan = Apply_Filter(image, locLaplacian_chuan)
...
img_loc_Laplacian_Bien_The_1 = Apply_Filter(image, locLaplacian_Bien_The_1)
